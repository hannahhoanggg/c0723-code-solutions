...
export default function Todos() {
  ...
  async function addTodo(newTodo) {
    try {
      ...
      const res = await fetch('/api/todos', postRequest);
      if (!res.ok) throw new Error(`Error: , status code: ${res.status}`);
      const resTodo = await res.json();
      const updatedTodos = todos.concat(resTodo);
  ...
  async function toggleCompleted(todoId) {
    try {
      ...
      if (!todo) throw new Error('Error, todo not found.', todoId);
      const patchRequest = {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
      ...
      const res = await fetch(`/api/todos/${todoId}`, patchRequest);
      if (!res.ok) throw new Error(`Error: , status code: ${res.status}`);
